...
Built for `linux/amd64` and pushed to `quay.io/rcook`.

## Local Dockerfiles (this repo — openrag)

| Dockerfile | Image | Original Base | UBI9 Base |
|---|---|---|---|
| `Dockerfile` | `quay.io/rcook/openrag-opensearch:latest` | `opensearchproject/opensearch:3.2.0` (AlmaLinux) | `ubi9/ubi:latest` (both stages, tarball install) |
| `Dockerfile.backend` | `quay.io/rcook/openrag-backend:latest` | `python:3.13-slim` (Debian) | `ubi9/ubi-minimal:latest` (build + runtime) |
| `Dockerfile.frontend` | `quay.io/rcook/openrag-frontend:latest` | `node:20.20.0-slim` (Debian) | `ubi9/nodejs-20:latest` (build + runtime) |
| `Dockerfile.langflow` | `quay.io/rcook/openrag-langflow:latest` | `langflowai/langflow:1.8.0` (Debian) | `ubi9/ubi:latest` (build) + `ubi9/ubi-minimal` (runtime) |
